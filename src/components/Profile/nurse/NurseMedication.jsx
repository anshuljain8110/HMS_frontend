...
function NurseMedication() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://hms-backend-toag.onrender.com/user/get-users");
  ...
  const handleAddMedication = async(e) =>{
    e.preventDefault() ;
    await axios.post(`https://hms-backend-toag.onrender.com/user/add-medications`,{name ,  frequency , dosage, email:changePatient})
    .then((res) =>{
      Swal.fire({
        title: "Success",
        icon: "success",
        ...
        <div className="overflow-auto  justify-center items-center w-[70%] ms-24 p-4 flex flex-col ">
          <form className="flex flex-col w-[60%] gap-5" action="">
            <div>
              ...
              <select
  id="doctors"
  value={changePatient} // Ensure controlled input
  onChange={(e) => setChangePatient(e.target.value)}
  className="flex h-10 w-[90%] rounded-md border border-gray-300 bg-transparent px-3 py-2 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-400 focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50"
>
  <option value="">Select a patient</option> {/* Default option */}
